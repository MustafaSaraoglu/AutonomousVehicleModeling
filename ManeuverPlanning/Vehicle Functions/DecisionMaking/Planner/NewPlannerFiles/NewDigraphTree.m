...
% parent
    ...
    methods(Static)
        ...
        % Expand the tree, add a new state node and connect it to its parent
            ...
            childID = NewDigraphTree.getNewID();
            
            child = NewDigraphTree.getNodeName(childID, state);
            dG = NewDigraphTree.connect(dG, parent, child, edgeName, colorNode, colorEdge);
        end
        ...
        % Higlight min/max child node and edge to child and update value of parent node in digraph
            ...
            dG = NewDigraphTree.changeNodeColor(dG, childNode, colorHighlight);
            dG = NewDigraphTree.changeEdgeColor(dG, parentNode, childNode, colorHighlight);

            [dG, parentNode] = NewDigraphTree.updateNodeValue(dG, parentNode, value);
        end
        ...
        % Update a nodes value and change the node name accordingly
            ...
            dG = NewDigraphTree.changeNodeName(dG, node, updatedNode);
        end
