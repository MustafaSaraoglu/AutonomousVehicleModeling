    ...
    methods
        ...
        % Get the decisions for a driving mode for different accelerations
            ...
            for acc = acc_lower:1:acc_upper 
                ...
                [~, futureOrientation] = Frenet2Cartesian(trajectory.s(end), trajectory.d(end), ...
                                                         obj.RoadTrajectory);
                futureState = State(trajectory.s(end), trajectory.d(end), futureOrientation, ...
                                    trajectory.velocity(end));
                
                % Discrete trajectory
                occupiedCells = Continuous2Discrete(obj.spaceDiscretisation, trajectory.s, ...
                                                    trajectory.d, trajectory.time);
        ...
        % Get the decisions for a lane change for different maneuver times
            ...
            for durManeuver = dur_lower:1:obj.Th
                ...
                occupiedCells = Continuous2Discrete(obj.spaceDiscretisation, trajectoryFrenet.s, ...
                                                    trajectoryFrenet.d, trajectoryFrenet.time);
        ...
        % Get the decision to keep lane for other vehicle    
            ...
            futureState_min = State(trajectory_min.s(end), d, 'don''t care', trajectory_min.velocity(end));

             % Future state prediction (Max)
            futureState_max = State(trajectory_max.s(end), d, 'don''t care', trajectory_max.velocity(end));
            
            futureStates_KL = [futureState_min, futureState_max];

            TS = obj.calculateTS_Other(trajectory_min.s, trajectory_min.d, trajectory_max.s, ...
                                       trajectory_min.d, trajectory_min.time);
        ...
        % Get the decision to change lane for other vehicle for static maneuver with a=0, T=4s  
            ...
            if trajectoryFrenet_min.isFeasible()    
                ...
                if trajectoryFrenet_max.isFeasible()
                    ...
                    TS = obj.calculateTS_Other(trajectoryFrenet_min.s, ...
                                               trajectoryFrenet_min.d, ...
                                               trajectoryFrenet_max.s, ...
                                               trajectoryFrenet_max.d, trajectoryFrenet_min.time);
