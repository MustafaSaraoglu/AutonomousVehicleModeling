function [s,d] = Cartesian2Frenet(currentTrajectory,Vpos_C)
    ...
    %d:                     nx1 array: Lateral offset - positive d means to the left of the reference road 
    ...
    if currentTrajectory(3,1) == 0  % Straight road := radian value equals to zero
        ...
        s = (dot(posVector', route_UnitVector'*ones(1, size(posVector, 1))))'; % the projection of posVector on route_UnitVector
        ...
        d = (dot(posVector', normalVector'*ones(1, size(posVector, 1))))'; % the projection of posVector on sideVector - positive d value means to the left
        ...
        angle = real(acos((dot(posVector', startPointVector'*ones(1, size(posVector, 1))))'./(l*r))); % Angle between vectors
